...
DATA  WK05;
  ...
  IF  ^MISSING(VAR36) THEN DO;
    DSTERM = "SECONDARY CANCER";
    DSDECOD = "SECONDARY CANCER";
    DSSTDTC = PUT(VAR36,IS8601DA.);
  END ;
  ELSE DELETE;
RUN ;
...
DATA  WK07;
  ...
  IF  KINDEX(VAR27,"ƒvƒƒgƒR[ƒ‹ˆá”½")>0 OR KINDEX(VAR27,"•s“KŠi«")>0 THEN DO;
    DSTERM = "PROTOCOL VIOLATION";
    DSDECOD = "PROTOCOL VIOLATION";
    DSSTDTC = PUT(VAR26,IS8601DA.);
  END ; 
  ELSE DELETE;
RUN ;
...
DATA  WK08;
  ...
  IF  KINDEX(VAR27,"’S“–ˆã‚É‚æ‚è’†Ž~")>0 THEN DO;
    DSTERM = "STUDY TERMINATED BY SPONSOR";
...
DATA  WK09;
  ...
  IF  KINDEX(VAR27,"Ž¡—ÃƒŠƒXƒNFŒQ")>0 THEN DO;
    DSTERM = "MET PROTOCOL DEFINED DISCONTINUATION CRITERIA";
    DSDECOD = "MET PROTOCOL DEFINED DISCONTINUATION CRITERIA";
    DSSTDTC = PUT(VAR26,IS8601DA.);
  END ; 
  ELSE DELETE;
RUN ;
